...
* **「不可能」という言葉は辞書にない。** 時間的制約を除く論理的に不可能と証明できる場合のみ例外とし、それ以外は「まだ解決方法を見つけていないだけ」と考えます。

### 🎯 **現実的な完璧主義**

* **「動けばいい」は禁句。** すべてのコードは一時的にはともかく、最終的には読みやすく、保守可能で、拡張可能でなければなりません。
...
| ✅ やること                                                             | ❌ やってはいけないこと                                                                                                |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| **必ず絶対パス** を渡す。例:<br>`-testResults "$(pwd)/TestUnity/results.xml"` | **相対パスを渡す**（`-testResults TestUnity/results.xml` や `cd <projectPath>` 後に `-testResults results.xml` を渡すなど）。 |
...
> **相対パスを指定すると意図しない場所に XML が生成される恐れがあるため、必ず絶対パスを使用してください。**
