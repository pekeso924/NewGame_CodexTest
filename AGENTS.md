...
3. Unity プロジェクトの取り扱い
   ...
   5. Git 操作の注意点
   6. **CI で毎回起こる「意図しない差分」エラーを 0 にする手順** ←★NEW
...
1. **Unity を一度バッチモードで起動し、全自動生成物を吐かせる**
   ...
   ```bash
   /opt/unity/Editor/Unity -batchmode -nographics -projectPath TestUnity -quit
   ```
...
4. **生成系フォルダ/テスト/結果 XML は .gitignore で除外**
   ...
   /**/results-*.xml
   ```
5. CI の最終ステップで `git diff --exit-code` を走らせ、
   **差分ゼロを保証** ― もし赤くなったら上記手順を踏み直すだけで解決できます。
