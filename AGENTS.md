...
* **⏰ Unity実行時の待機と停止の重要な注意事項**
  * **Unity を実行した後、処理が完了するまで数分から十数分かかる場合があります。これは正常な動作です。**
  * **基本的には BatchBoot により自動で停止するため、慌てて手動停止しないでください。**
  * **ただし、以下の異常な状況が疑われる場合のみ、手動で停止（Ctrl+C等）してください：**
    * エラーメッセージが大量に出力され続けている
    * 同じログが無限に繰り返し出力されている
    * 30分以上経過しても一切進展がない
    * CPU使用率が異常に高い状態が長時間続いている
  * **正常な処理中は「待つ」ことが最も重要です。途中で停止すると `.meta` ファイルの生成が不完全になり、プロジェクトが破損する可能性があります。**
